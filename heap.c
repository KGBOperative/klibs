...
heap *h_copy(heap *h, size_t m) {
    ...
    new_h->m = m;
    new_h->as = malloc(m * sizeof(void *));

    new_h->n = h->n > m ? m : h->n;
    new_h->cmp = h->cmp;
    memcpy(new_h->as, h->as, new_h->n * sizeof(void *));
...
bool h_push(heap *h, void *a) {
    ...
    if (h->n == h->m) {
        ...
        memmove(new_fs, h->as, h->m * sizeof(void *));
        free(h->as);
        h->as = new_fs;
        h->m = new_size;
    }
...
void h_foreach(heap *h, void *(*func)(void *)) {
    size_t i;
    for (i = 1; i < h->n; i++) {
        h->as[i] = (func)(h->as[i]);
    }
...
size_t h_reduce(heap *h, bool (*func)(void *)) {
    size_t i, c = 1;
